...
fn test_exif_apply_and_read_with_focal_length() {
  ...
  assert!(
    read_result.is_ok(),
    "Failed to read EXIF data after applying: {:?}",
    read_result.err()
  );
  ...
  let has_make = exif_data
    .iter()
    .any(|(key, value)| key.contains("Make") && value.contains("Test"));
  let has_model = exif_data
    .iter()
    .any(|(key, value)| key.contains("Model") && value.contains("Camera"));
  let has_iso = exif_data
    .iter()
    .any(|(key, value)| key.contains("ISO") && value.contains("800"));
...
fn test_exif_apply_and_read_without_focal_length() {
  ...
  let has_iso = exif_data
    .iter()
    .any(|(key, value)| key.contains("ISO") && value.contains("200"));
