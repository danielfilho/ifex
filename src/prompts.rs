...
impl PromptUtils {
  ...
  /// Returns None if the user cancels the operation.
  pub fn prompt_text_with_default(message: &str, default: &str) -> Result<Option<String>, Box<dyn std::error::Error>> {
  ...
  /// Returns None if the user cancels the operation.
  pub fn prompt_number_with_default<T>(message: &str, default: T) -> Result<Option<T>, Box<dyn std::error::Error>>
  where
    T: std::str::FromStr + Clone + std::fmt::Display,
    T::Err: std::fmt::Display + std::fmt::Debug + Send + Sync + 'static,
  {
  ...
  pub fn format_table_header(tag_width: usize, value_width: usize) -> String {
    ...
    format!(
      " {} │ {} \n{}",
      format!("{:width$}", tag_header, width = tag_width).cyan(),
      format!("{:width$}", value_header, width = value_width).cyan(),
  ...
  pub fn format_table_row(tag: &str, value: &str, tag_width: usize, value_width: usize) -> String {
    format!(
      " {} │ {} ",
      format!("{:width$}", tag, width = tag_width).yellow(),
  ...
  pub fn format_table_footer(_tag_width: usize, _value_width: usize) -> String {
    "".to_string()
  }
}
